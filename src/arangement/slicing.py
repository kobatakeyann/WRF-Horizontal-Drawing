...
from replacement import DimensionArrayReplacement
...
class Slicing(DimensionArrayReplacement):
    def slice_in_lat_dir(self) -> None:
        self.data = self.data.isel(south_north=slice(LAT_LON_RANGE["lat"]))

    def slice_in_lon_dir(self) -> None:
        self.data = self.data.isel(west_east=slice(LAT_LON_RANGE["lon"]))

    def slice_in_time_dir(self, datetime: datetime) -> Dataset:
        return self.data.isel(Time=datetime)
